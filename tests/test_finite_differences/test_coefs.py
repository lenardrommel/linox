...
from linox.finite_differences.coefs import (
    calc_coefs,
    coefficients,
...
def test_order2_acc2(analytic_inv):
    for analytic_inv in [True, False]:
        ...
        np.allclose(np.array([1.0, -2.0, 1.0]), coefs)
        offs = c["center"]["offsets"]
        np.allclose(np.array([-1, 0, 1]), offs)

        coefs = c["forward"]["coefficients"]
        np.allclose(np.array([2, -5, 4, -1]), coefs)
        offs = c["forward"]["offsets"]
        np.allclose(np.array([0, 1, 2, 3]), offs)

        coefs = c["backward"]["coefficients"]
        np.allclose(np.array(([2, -5, 4, -1])[::-1]), coefs)
        offs = c["backward"]["offsets"]
        np.allclose(np.array([-3, -2, -1, 0]), offs)
