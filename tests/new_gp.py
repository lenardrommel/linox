...
def params_from_structure(structure: StructureConfig) -> Params:
    ...
    params["output_scale"] = _to_jax_array(-3.01)
    params["noise_variance"] = _to_jax_array(-1.0)
...
class ModularGPPrior:
    ...
    def predict(
        ...
        alpha = lo.lsolve(K_train, y_flat)
    ...
    def negative_log_likelihood(
        ...
        alpha = lo.lsolve(K, y_flat)

        quad = 0.5 * (y_flat * alpha).sum()
        _, logdet = lo.slogdet(K)
        logdet = 0.5 * jnp.sum(logdet)
        const = 0.5 * K.shape[0] * jnp.log(2 * jnp.pi)
        return quad + logdet + const


# -----------------------------------------------------------------------------
# Local get_structure_config_for_data function
# -----------------------------------------------------------------------------
def get_structure_config_for_data(data_name: str) -> StructureConfig:
    """Get appropriate structure configuration for a given dataset."""
    if "1D" in data_name or "1d" in data_name:
    ...
    elif "2D" in data_name or "2d" in data_name:
        return StructureConfig(
            spatial_dims=[
...
def solve_2d(
    operator_inputs,  # train functions (N_train, nx, ny)
    spatial_inputs,  # train grid       (nx, ny, 2)
    outputs,  # train targets    (N_train * nx * ny,)
    operator_inputs_test,  # test functions   (N_test, nx, ny)
    spatial_inputs_test,  # test grid (same res as train) -> optional hier
    outputs_test,  # test targets (nur für plots/vergleich)
    spatial_inputs_plot,  # plot grid        (nx_plot, ny_plot, 2)
    nx,
    ny,
    nx_plot,
    ny_plot,
    ...
    # 2) Hyperparameter-Defaults (Dict statt Klasse)
    params = params_from_structure(structure_config)

    # 3) Trainings-Kernel + Noise, lineares System lösen
    K_train = prior.build_kernel(operator_inputs, spatial_inputs, params)
    y_flat = outputs.reshape(-1, 1)
    alpha = lo.lsolve(K_train, y_flat)

    # 4) Cross-Kernel (Test x Train), Vorhersage-Mittel
    K_cross = prior.build_kernel(
        operator_inputs_test,
        spatial_inputs_plot,
        params,
        x2=operator_inputs,
        grid2=spatial_inputs,
    )
    pred_mean = K_cross @ alpha
    pred_mean_flat = pred_mean.reshape(N_test, -1)

    K_test = prior.build_kernel(operator_inputs_test, spatial_inputs_plot, params)
    v = lo.linverse(K_train) @ K_cross.T
    pred_cov = K_test - K_cross @ v
    ...
    # sample
    samples = prior.sample_posterior(
        jax.random.PRNGKey(0),
        operator_inputs,  # training inputs
        outputs,  # training observations
        spatial_inputs,  # training grid
        operator_inputs_test,  # test inputs
        spatial_inputs_plot,  # test grid
        params,
        size=(3,),
        dtype=jnp.float64,
    )

    return pred_mean_flat, pred_cov, alpha, samples
...
def run_2d():
    ...
    pred_mean, pred_cov, alpha, samples = solve_2d(
        operator_inputs,
        spatial_inputs,
        outputs,
        operator_inputs_test,
