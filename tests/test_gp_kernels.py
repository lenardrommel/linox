...
import pytest
import linox as lo
from linox.kernels.kernel import Matern32Kernel, L2InnerProductKernel
...
def get_kernel_fn(name, **params):
    if name == "M32":
        return Matern32Kernel(learnable=False, lengthscale=params.get("lengthscale", 1.0))
    elif name == "L2":
...
def build_1d_problem(n_spatial=10, n_time=5):
    ...
    K_final = lo.IsotropicAdditiveLinearOperator(jnp.array(params["noise_variance"]), K_scaled)
...
def build_2d_problem(n_spatial=8, n_time=3):
    ...
    K_function = lo.kernel_operator(ku_fn, u, is_stationary=False) # Not stationary because H1/L2 part
...
class TestGPKernels1D:
    ...
    def test_slogdet(self):
        ...
        K_dense = K.todense()
        sign_d, logdet_d = jnp.linalg.slogdet(K_dense)
...
class TestGPKernels2D:
    ...
    def test_prediction_pipeline_ops(self):
    ...
    def test_sample_posterior_ops(self):
