...
from linox._arithmetic import lmul, lsolve
from linox._matrix import Matrix


def coefficients(deriv, acc=None, offsets=None, symbolic=False, analytic_inv=False):  # noqa: ANN001, ANN201
    ...
    if offsets:
        if deriv >= len(offsets):
            raise ValueError(  # noqa: TRY003
                f"can not compute derivative of order {deriv} using {len(offsets)} offsets."  # noqa: EM102
            )
        return calc_coefs(deriv, offsets, symbolic, analytic_inv)

    if acc is None:
        raise ValueError("either acc or offsets has to be given")  # noqa: EM101, TRY003
...
def compute_inverse_Vandermonde(column, offsets, symbolic):
    ...
    if symbolic:
        ...
        minus = lambda x, arr: x - arr  # noqa: E731, FURB118
    ...
    return jnp.array(inv_vandermonde_column) * math.factorial(column)
...
def calc_coefs(deriv, offsets, symbolic=False, analytic_inv=False):
    if analytic_inv:
        ...
        if symbolic:
            coefs = lsolve(matrix, rhs)
            coefs = list(tuple(coefs)[0])
        else:
            coefs = jnp.linalg.solve(matrix, rhs)
...
def _build_matrix(offsets):
    ...
    n = len(offsets)
    powers = jnp.arange(n)[:, None]
    offsets_expanded = offsets[None, :]  # Shape: (1, n)
    A = jnp.power(offsets_expanded, powers)
    return A


def _build_rhs(offsets, deriv, symbolic=False, dtype=jnp.float32):
    """The right hand side of the equation system matrix"""

    b = [0 for _ in offsets]
    b[deriv] = math.factorial(deriv)
    if symbolic:
        return Matrix(b, dtype=dtype)  # noqa: ANN202
    else:
        return jnp.array(b, dtype=dtype)


def _build_matrix_non_uniform(p, q, coords, k, dtype=jnp.float32):
