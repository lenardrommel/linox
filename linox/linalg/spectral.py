...
def eigh(
    ...
    **kwargs,
    ...
    Args:
        ...
        subset_by_index: Range of indices.
        method: "auto", "exact", "lanczos".
    """
    ...
    if method == "auto":
        ...
        n = op.shape[0]
        if n <= get_max_dense_n() and k is None:
            method = "exact"
        elif k is not None and k < n // 2:
            method = "lanczos"
        else:
            method = "exact"

    if method == "exact":
        # Pass k to leigh if it supports it? leigh implementation usually does full
        # But leigh in _arithmetic computes full.
        # We might need to slice result if k is provided.
        evals, evecs = leigh(op)
    ...
    if method == "lanczos":
        ...
        # Extract v0
        v0 = kwargs_.pop("v0", None)
        if v0 is None:
            import jax.numpy as jnp

            # Use key if available, otherwise ones
            # key needs to be popped too if we use it? Or keep it?
            # lanczos_eigh doesn't use key.
            key = kwargs_.get("key")
            if key is not None:
                import jax

                v0 = jax.random.normal(key, (op.shape[0],))
            else:
                v0 = jnp.ones(op.shape[0])

        # Extract num_iters
        # Default: 2*k or 20
        default_iters = (2 * k) if k else 20
        num_iters = kwargs_.pop("num_iters", default_iters)

        return lanczos_eigh(op, v0, num_iters, k=k, **kwargs_)
